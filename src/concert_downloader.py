...
def wdr3_scraper(url: str, file: str) -> int:
    ...
    try:
        if not file.endswith(".mp3"):
            raise NameError
        # verificare ed tentare d'aprire initial url
        res = requests.get(url=TestURL(url=url).url)
